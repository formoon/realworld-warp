...
pub fn route_create_user() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::path!("api" / "users")
        .and(warp::post())
        .and(warp::body::json())
...
pub async fn handler_put_user(user: UpdateUser, auth: Auth) -> Result<impl warp::Reply, Infallible> {
