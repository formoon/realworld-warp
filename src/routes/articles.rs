...
pub fn route_new_article() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
...
pub async fn handler_new_article(new_article: NewArticle, auth: Auth) -> Result<impl warp::Reply, Infallible> {
...
pub fn route_get_articles() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::get()
...
pub async fn handler_get_articles(params: FindArticles, auth: Auth) -> Result<impl warp::Reply, Infallible> {
...
// --------------------------------------------------------------------------------------------------------
pub fn route_get_article() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::get()
...
// --------------------------------------------------------------------------------------------------------
pub fn route_get_comments() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::get()
        .and(warp::path!("api" / "articles" / String / "comments"))
        .and(routes::extract_auth_head())
        .and_then(handler_get_comments)
}
pub async fn handler_get_comments(slug: String, auth: Auth) -> Result<impl warp::Reply, Infallible> {
...
pub fn route_update_article() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::put()
        .and(warp::path!("api" / "articles" / String))
        .and(warp::body::json())
        .and(routes::extract_auth_head())
...
pub async fn handler_update_article(slug:String, article: UpdateArticle, auth: Auth) 
...
pub fn route_new_comment() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::post()
        .and(warp::path!("api" / "articles" / String / "comments"))
        .and(warp::body::json())
        .and(routes::extract_auth_head())
...
pub async fn handler_new_comment(slug:String, new_comment: NewComment, auth: Auth) 
    -> Result<impl warp::Reply, Infallible> {
...
// --------------------------------------------------------------------------------------------------------
pub fn route_delete_comment() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::delete()
        .and(warp::path!("api" / "articles" / String / "comments" / i32))
        .and(routes::extract_auth_head())
        .and_then(handler_delete_comment)
}
pub async fn handler_delete_comment(slug: String, id: i32, auth: Auth) -> Result<impl warp::Reply, Infallible> {
...
// --------------------------------------------------------------------------------------------------------
pub fn route_delete_article() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::delete()
        .and(warp::path!("api" / "articles" / String))
        .and(routes::extract_auth_head())
        .and_then(handler_delete_article)
}
pub async fn handler_delete_article(slug: String, auth: Auth) -> Result<impl warp::Reply, Infallible> {
...
// --------------------------------------------------------------------------------------------------------
pub fn route_favorite_article() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::post()
        .and(warp::path!("api" / "articles" / String / "favorite"))
        .and(routes::extract_auth_head())
        .and_then(handler_favorite_article)
}
pub async fn handler_favorite_article(slug: String, auth: Auth) -> Result<impl warp::Reply, Infallible> {
...
// --------------------------------------------------------------------------------------------------------
pub fn route_unfavorite_article() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::delete()
        .and(warp::path!("api" / "articles" / String / "favorite"))
        .and(routes::extract_auth_head())
        .and_then(handler_unfavorite_article)
}
pub async fn handler_unfavorite_article(slug: String, auth: Auth) -> Result<impl warp::Reply, Infallible> {
...
pub fn route_get_articles_feed() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::get()
...
pub async fn handler_get_articles_feed(params: FeedArticles, auth: Auth) 
    -> Result<impl warp::Reply, Infallible> {
