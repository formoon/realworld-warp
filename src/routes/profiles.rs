...
fn to_profile_json(profile: Profile) -> Result<warp::reply::WithStatus<warp::reply::Json>, Infallible> {
...
pub fn route_get_profile() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::get()
        .and(warp::path!("api" / "profiles" / String))
        .and(routes::extract_auth_head())
        .and_then(handler_get_profile)
}
pub async fn handler_get_profile(username: String, auth: Auth) 
...
pub async fn handler_follow(username: String, auth: Auth) 
        -> Result<impl warp::Reply, Infallible> {
...
pub async fn handler_unfollow(username: String, auth: Auth) 
        -> Result<impl warp::Reply, Infallible> {
